...
<script>
	function fanhui(){ //返回上一页
		...
		var url = "<%=request.getContextPath()%>" + "/wechatServiceHistory.do?mainXQ&gongdanId=" + gongdanId + "&weixin=weixin";
	...
	function relizeIMG1(){ 	//图片大小自适应
       var phoneH = $(window).height();
	   var imageH = $("#myCarousel").width();	 //正方形图片
	   //console.log(imageH);
	   // console.log(phoneH);
	   if(imageH < phoneH){
			$("#myCarousel").css("margin-top",(phoneH-imageH)/2+"px");
	...
	function querygongdan(){ //加载图片
		var gongdanId = "<%=request.getAttribute("gongdanId")%>";
		var reqmsg="{'action':'QUERY_SERVICE_TASK_INFO_REQUEST','content':{\"serviceTaskPhotoListShow\":\"true\",\"id\":"+gongdanId+"}}";		
		  jQuery.ajax({
          type : "post",
          async:false,
            	  ...
            	  if (data.content != null) {
	            	 ...
	            	 var shuliang = 0;
 					 if(data.content.serviceTaskPhotoList != null && data.content.serviceTaskPhotoList != ""){
 					 	shuliang = data.content.serviceTaskPhotoList.length;
						jQuery.each(data.content.serviceTaskPhotoList, function(i, item) {
							...
							html += "<img style=\"width:100%;\"  src=\"" + item.photoUrl + "?random=" + Math.random() + "\">";
							//html += "<div class=\"carousel-caption\">" + (i+1) + "/" + shuliang + "  " + item.title + "</div>";
							html += "</div>"
