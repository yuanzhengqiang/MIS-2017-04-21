								...
								<div class="dataTables_wrapper form-inline">
									...
									<div class="row">
										<div class="col-sm-1" style="margin-bottom:5px;">
										 <c:if test="${dic_query_btn_control != 'yes' or dic_query_btn_show == 'yes'}">
									...
									<!-- 操作按钮开始 -->
									 <div class="row">
										<div class="col-sm-12">
											...
											<c:if test="${dic_del_btn_control != 'yes' or dic_del_btn_show == 'yes'}">
											...
											<c:if test="${dic_gx_btn_control != 'yes' or dic_gx_btn_show == 'yes'}">
...
<!-- 											<c:if test="${dic_add_btn_control != 'yes' or dic_add_btn_show == 'yes'}"> -->
				...
				<div class="block-flat">
					...
					<div class="content">
						<form class="form-horizontal group-border-dashed" action="#"
							...
							<input type="hidden" name="parentlinshi" id="parentlinshi" value="0">
...
$(document).ready(function(){
    ...
    $("#breadcrumb2").html("<li><a href=\"#\" onclick=\"javascript:query2('0',1);\">根目录</a></li>");	
	go2page("",1);
});
...
function changeData(data){
	...
	if (data.content != null) {
		jQuery.each(data.content.dataDicList, function(i, item) {
			...
			<c:if test="${dic_query_opt_control != 'yes' or dic_query_opt_show == 'yes'}">
			htmlcode += "<li onclick=\"query2('" + item.id + "',1)\"><a href=\"###\">查看子节点</a></li>";
			</c:if>	
			<c:if test="${dic_addself_opt_control != 'yes' or dic_addself_opt_show == 'yes'}">
			htmlcode += "<li onclick=\"addSelf('" + item.id + "')\"><a href=\"###\">新增同级节点</a></li>";
			</c:if>
			<c:if test="${dic_addson_opt_control != 'yes' or dic_addson_opt_show == 'yes'}">
			htmlcode += "<li onclick=\"addModule('" + item.id + "')\"><a href=\"###\">新增子节点</a></li>";
			</c:if>	
			<c:if test="${dic_edit_opt_control != 'yes' or dic_edit_opt_show == 'yes'}">
			htmlcode += "<li onclick=\"editModule('" + item.id + "')\"><a href=\"###\">编辑</a></li>";
			</c:if>	
			<c:if test="${dic_del_opt_control != 'yes' or dic_del_opt_show == 'yes'}">
...
function query2(id,pagenumber) {
	...
	//生成导航
	var htmlcode="<li><a href=\"#\" onclick=\"javascript:query2('0',1);\">根目录</a></li>";
	var val=["无","无"];
	if(id != ""){
    jQuery.ajax({
          type : "post",
     ...
     if(idSplit.length == nameSplit.length){
     	if(idSplit.length == 1){
     		htmlcode="";
     		htmlcode="<li><a href=\"#\" onclick=\"javascript:query2('0',1);\">根目录</a></li>";
     		if(nameSplit[0] != "无"){
