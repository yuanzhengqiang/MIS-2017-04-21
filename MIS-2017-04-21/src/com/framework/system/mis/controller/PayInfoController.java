...
import com.framework.system.mis.handler.PayInfoHandler;
import com.framework.system.mis.service.PayInfoService;
import com.framework.system.util.JsonUtil;

@RequestMapping("/payInfo")
@Controller
public class PayInfoController {
	private static Logger logger = Logger.getLogger(PayInfoController.class);
	/**
	 * 处理器
	 */
	private PayInfoHandler payInfoHandler = PayInfoHandler.getInstance();
	/**
	 * 服务类
	 */
	...
	public AjaxJson del(HttpServletRequest request) {
		...
		if (ids != null && !"".equals(ids)) {
			boolean falg = payInfoService.del(ids);
			if (falg) {
				result = "success";
				des = "删除成功";
			}
	...
	public String query(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String rep = payInfoHandler.doHandler(1, action, reqmsg, request,
					response);
