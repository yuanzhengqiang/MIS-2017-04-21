...
import com.framework.system.mis.handler.ServicePersonHandler;
import com.framework.system.mis.service.ServicePersonService;
...
public class ServicePersonController {
	private static Logger logger = Logger
			.getLogger(ServicePersonController.class);
	/**
	 * 处理器
	 */
	private ServicePersonHandler servicePersonHandler = ServicePersonHandler
			.getInstance();
	/**
	 * 服务类
	 */
	...
	public ModelAndView center(HttpServletRequest request) {
		ModelAndView mav = new ModelAndView("servicePersonManagement/servicePersonList");
		return mav;
	}
	...
	public AjaxJson del(HttpServletRequest request) {
		...
		if (ids != null && !"".equals(ids)) {
			boolean falg = servicePersonService.del(ids);
			if (falg) {
				result = "success";
				des = "删除成功";
			}
	...
	public String query(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String rep = servicePersonHandler.doHandler(1, action, reqmsg,
					request, response);
