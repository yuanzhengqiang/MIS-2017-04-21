...
import com.framework.system.mis.pack.OrderPack;
import com.framework.system.mis.parse.OrderParse;
...
public class OrderHandler extends BaseHandler {
	...
	public String doHandler(int type, String command, String reqStr,
		...
		try {
			...
			// List<OrderVO> orderList = (List<OrderVO>)
			// parseMap.get("orderList");

			Boolean parentOrderShow = (Boolean) parseMap.get("parentOrderShow");
			Boolean delParentOrder = (Boolean) parseMap.get("delParentOrder");
			Boolean delParentOrderList = (Boolean) parseMap
					.get("delParentOrderList");
			Boolean childOrderListShow = (Boolean) parseMap
			...
			if ("save".equals(action)) {
				...
				result = orderService.getById(id, parentOrderShow,
						childOrderListShow, servicePersonShow,
						medicalReportShow);
			} else if ("getListByCondition".equals(action)) {
				// // 根据数据权限 增加查询条件
				// queryMap = systemService.addDataRuleByRoles("OrderEntity",
				// queryMap, request);
				// result = orderService.getListByCondition(queryMap, orderList,
				// pageno, pagesize, servicePersonShow, medicalReportShow);
			} else if ("del".equals(action)) {
				result = orderService.del(id, delParentOrder,
						delChildOrderList, delServicePerson, delMedicalReport);
			} else if ("delList".equals(action)) {
				result = orderService.delList(queryMap, delParentOrderList,
						delChildOrderList, delServicePerson, delMedicalReport);
			}
			// 封装
