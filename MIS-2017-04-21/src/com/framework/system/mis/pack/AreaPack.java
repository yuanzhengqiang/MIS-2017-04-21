 ...
 *
 */
public class AreaPack {
	private static Logger logger = Logger.getLogger(AreaPack.class);
	private static AreaPack areaPack;
	 ...
	 * @return
	 */
	public static AreaPack getInstance() {
		if (areaPack == null) {
			areaPack = new AreaPack();
		}
		return areaPack;
	}
	...
	private Map<String, Object> packByJson(String action, Object request,Object entity) {
		...
		try {
			if ("save".equals(action)) {
				actionBack="ADD_AREA_INFO_RESPONSE";	
				...
				AreaEntity area = (AreaEntity)entity;
				if(area!=null){
					contentBack = new JSONObject();
					contentBack.put("id", area.getId());
					
									}
			} else if ("getById".equals(action)) {
				actionBack="QUERY_AREA_INFO_RESPONSE";		
				resultBack="100";
				desBack="success";
				AreaEntity area = (AreaEntity)request;
				if(area!=null){
					contentBack = JSONObject.fromObject(area);
									}
			} else if ("getListByCondition".equals(action)) {
				actionBack="QUERY_AREA_LIST_RESPONSE";
				resultBack="100";
				desBack="success";
				...
				actionBack="DEL_AREA_INFO_REQUEST";	
				if(request.equals(true)){
					resultBack="100";
					desBack="success";
				}else if(request.equals(false)){
				...
				actionBack="DEL_AREA_LIST_REQUEST";	
				if(request.equals(true)){
					resultBack="100";
					desBack="success";
				}else if(request.equals(false)){
