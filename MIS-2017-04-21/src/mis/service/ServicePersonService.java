...
public class ServicePersonService {
	...
	public ServicePersonEntity getById(Integer id) {
		...
		if (id != null) {
			obj = (ServicePersonEntity) dbManager.getById(id,
	 ...
	 * @return
	 */
	public List<ServicePersonEntity> getListByCondition(
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		QueryCondition qc = changeMapToQc(queryMap);
		pagelist = dbManager.queryByCondition(ServicePersonEntity.class, qc,
				pageno, pagesize);
		return pagelist;
	}
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(ServicePersonEntity.class, qc,
	...
	public boolean del(String ids) {
		...
		if (ids != null && !"".equals(ids.trim())) {
			...
			try {
				tx.beginTransaction();
				QueryCondition qc = new QueryCondition(ServicePersonEntity.ID,
						QueryCondition.in, ids);
				result = dbManager.delByConditionsNoTransaction(
						ServicePersonEntity.class, qc);
				tx.commitAndClose();
			} catch (Exception e) {
				logger.error("数据库提交失败！");
				logger.error(e);
	...
	private QueryCondition changeMapToQc(Map<String, Object> queryMap) {
		...
		QueryCondition qc = new QueryCondition(ServicePersonEntity.ID,
				QueryCondition.gt, "0");
		if (id != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ID,
		...
		if (name != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.NAME,
		...
		if (gender != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.GENDER,
		...
		if (headPortrait != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.HEAD_PORTRAIT, QueryCondition.eq,
					headPortrait));
		}
		if (headPortrait_like != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.HEAD_PORTRAIT, QueryCondition.like,
					headPortrait_like));
		}
		if (headPortrait_isNull != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.HEAD_PORTRAIT, QueryCondition.isNull,
					headPortrait_isNull));
		}
		if (headPortrait_isNotNull != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.HEAD_PORTRAIT,
					QueryCondition.isNotNull, headPortrait_isNotNull));
		}
		if (headPortrait_in != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.HEAD_PORTRAIT, QueryCondition.in,
					headPortrait_in));
		}
		if (birthday != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.BIRTHDAY,
					QueryCondition.eq, birthday));
		}
		if (birthday_like != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.BIRTHDAY,
					QueryCondition.like, birthday_like));
		}
		if (birthday_isNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.BIRTHDAY,
					QueryCondition.isNull, birthday_isNull));
		}
		if (birthday_isNotNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.BIRTHDAY,
					QueryCondition.isNotNull, birthday_isNotNull));
		}
		if (birthday_in != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.BIRTHDAY,
					QueryCondition.in, birthday_in));
		}
		if (academic != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ACADEMIC,
					QueryCondition.eq, academic));
		}
		if (academic_like != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ACADEMIC,
					QueryCondition.like, academic_like));
		}
		if (academic_isNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ACADEMIC,
					QueryCondition.isNull, academic_isNull));
		}
		if (academic_isNotNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ACADEMIC,
					QueryCondition.isNotNull, academic_isNotNull));
		}
		if (academic_in != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.ACADEMIC,
					QueryCondition.in, academic_in));
		}
		if (contact != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.CONTACT,
					QueryCondition.eq, contact));
		}
		if (contact_like != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.CONTACT,
					QueryCondition.like, contact_like));
		}
		if (contact_isNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.CONTACT,
					QueryCondition.isNull, contact_isNull));
		}
		if (contact_isNotNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.CONTACT,
					QueryCondition.isNotNull, contact_isNotNull));
		}
		if (contact_in != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.CONTACT,
					QueryCondition.in, contact_in));
		}
		if (wechatNum != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.WECHAT_NUM,
					QueryCondition.eq, wechatNum));
		}
		if (wechatNum_like != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.WECHAT_NUM,
					QueryCondition.like, wechatNum_like));
		}
		if (wechatNum_isNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.WECHAT_NUM,
					QueryCondition.isNull, wechatNum_isNull));
		}
		if (wechatNum_isNotNull != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.WECHAT_NUM,
					QueryCondition.isNotNull, wechatNum_isNotNull));
		}
		if (wechatNum_in != null) {
			qc.andCondition(new QueryCondition(ServicePersonEntity.WECHAT_NUM,
					QueryCondition.in, wechatNum_in));
		}
		if (wechatQrCode != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.WECHAT_QR_CODE, QueryCondition.eq,
					wechatQrCode));
		}
		if (wechatQrCode_like != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.WECHAT_QR_CODE, QueryCondition.like,
					wechatQrCode_like));
		}
		if (wechatQrCode_isNull != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.WECHAT_QR_CODE, QueryCondition.isNull,
					wechatQrCode_isNull));
		}
		if (wechatQrCode_isNotNull != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.WECHAT_QR_CODE,
					QueryCondition.isNotNull, wechatQrCode_isNotNull));
		}
		if (wechatQrCode_in != null) {
			qc.andCondition(new QueryCondition(
					ServicePersonEntity.WECHAT_QR_CODE, QueryCondition.in,
					wechatQrCode_in));
		}
		return qc;
	}
