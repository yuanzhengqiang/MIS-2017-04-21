...
public class MedicalItemService {
	...
	public MedicalItemEntity getById(Integer id) {
		...
		if (id != null) {
			obj = (MedicalItemEntity) dbManager.getById(id,
	 ...
	 * @return
	 */
	public List<MedicalItemEntity> getListByCondition(
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		QueryCondition qc = changeMapToQc(queryMap);
		pagelist = dbManager.queryByCondition(MedicalItemEntity.class, qc,
				pageno, pagesize);
		return pagelist;
	}
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(MedicalItemEntity.class, qc,
	...
	public boolean del(Integer id) {
		...
		if (id != null && id > 0) {
			...
			try {
				tx.beginTransaction();
				result = dbManager
						.delNoTransaction(id, MedicalItemEntity.class);
				tx.commitAndClose();
			} catch (Exception e) {
				logger.error("数据库提交失败！");
				logger.error(e);
	...
	public boolean del(String ids) {
		...
		if (ids != null && !"".equals(ids.trim())) {
			...
			try {
				tx.beginTransaction();
				QueryCondition qc = new QueryCondition(MedicalItemEntity.ID,
						QueryCondition.in, ids);
				result = dbManager.delByConditionsNoTransaction(
						MedicalItemEntity.class, qc);
				tx.commitAndClose();
			} catch (Exception e) {
				logger.error("数据库提交失败！");
				logger.error(e);
	...
	private QueryCondition changeMapToQc(Map<String, Object> queryMap) {
		...
		Object mattersNeedAttention_like = queryMap
				.get("mattersNeedAttention_like");
		Object mattersNeedAttention_isNull = queryMap
				.get("mattersNeedAttention_isNull");
		Object mattersNeedAttention_isNotNull = queryMap
				.get("mattersNeedAttention_isNotNull");
		Object mattersNeedAttention_in = queryMap
				.get("mattersNeedAttention_in");
		
		Object instructions = queryMap.get("instructions");
		Object instructions_like = queryMap.get("instructions_like");
		Object instructions_isNull = queryMap.get("instructions_isNull");
		Object instructions_isNotNull = queryMap.get("instructions_isNotNull");
		Object instructions_in = queryMap.get("instructions_in");

		QueryCondition qc = new QueryCondition(MedicalItemEntity.ID,
				QueryCondition.gt, "0");
		if (id != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ID,
		...
		if (itemName != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ITEM_NAME,
					QueryCondition.eq, itemName));
		}
		if (itemName_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ITEM_NAME,
					QueryCondition.like, itemName_like));
		}
		if (itemName_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ITEM_NAME,
					QueryCondition.isNull, itemName_isNull));
		}
		if (itemName_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ITEM_NAME,
					QueryCondition.isNotNull, itemName_isNotNull));
		}
		if (itemName_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.ITEM_NAME,
					QueryCondition.in, itemName_in));
		}
		if (category != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.eq, category));
		}
		if (category_gt != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.gt, category_gt));
		}
		if (category_ge != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.ge, category_ge));
		}
		if (category_lt != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.lt, category_lt));
		}
		if (category_le != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.le, category_le));
		}
		if (category_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.CATEGORY,
					QueryCondition.in, category_in));
		}
		if (testWay != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_WAY,
					QueryCondition.eq, testWay));
		}
		if (testWay_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_WAY,
					QueryCondition.like, testWay_like));
		}
		if (testWay_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_WAY,
					QueryCondition.isNull, testWay_isNull));
		}
		if (testWay_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_WAY,
					QueryCondition.isNotNull, testWay_isNotNull));
		}
		if (testWay_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_WAY,
					QueryCondition.in, testWay_in));
		}
		if (testPurpose != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_PURPOSE,
					QueryCondition.eq, testPurpose));
		}
		if (testPurpose_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_PURPOSE,
					QueryCondition.like, testPurpose_like));
		}
		if (testPurpose_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_PURPOSE,
					QueryCondition.isNull, testPurpose_isNull));
		}
		if (testPurpose_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_PURPOSE,
					QueryCondition.isNotNull, testPurpose_isNotNull));
		}
		if (testPurpose_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.TEST_PURPOSE,
					QueryCondition.in, testPurpose_in));
		}
		if (selectDes != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.SELECT_DES,
					QueryCondition.eq, selectDes));
		}
		if (selectDes_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.SELECT_DES,
					QueryCondition.like, selectDes_like));
		}
		if (selectDes_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.SELECT_DES,
					QueryCondition.isNull, selectDes_isNull));
		}
		if (selectDes_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.SELECT_DES,
					QueryCondition.isNotNull, selectDes_isNotNull));
		}
		if (selectDes_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.SELECT_DES,
					QueryCondition.in, selectDes_in));
		}
		if (price != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.PRICE,
					QueryCondition.eq, price));
		}
		if (price_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.PRICE,
					QueryCondition.like, price_like));
		}
		if (price_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.PRICE,
					QueryCondition.isNull, price_isNull));
		}
		if (price_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.PRICE,
					QueryCondition.isNotNull, price_isNotNull));
		}
		if (price_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.PRICE,
		...
		if (des != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
		...
		if (instructions != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
					QueryCondition.eq, instructions));
		}
		if (instructions_like != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
					QueryCondition.like, instructions_like));
		}
		if (instructions_isNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
					QueryCondition.isNull, instructions_isNull));
		}
		if (instructions_isNotNull != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
					QueryCondition.isNotNull, instructions_isNotNull));
		}
		if (instructions_in != null) {
			qc.andCondition(new QueryCondition(MedicalItemEntity.DES,
					QueryCondition.in, instructions_in));
		}
		//----------------------------------------------------------
		
		if (mattersNeedAttention != null) {
			qc.andCondition(new QueryCondition(
					MedicalItemEntity.MATTERS_NEED_ATTENTION,
					QueryCondition.eq, mattersNeedAttention));
		}
		if (mattersNeedAttention_like != null) {
			qc.andCondition(new QueryCondition(
					MedicalItemEntity.MATTERS_NEED_ATTENTION,
					QueryCondition.like, mattersNeedAttention_like));
		}
		if (mattersNeedAttention_isNull != null) {
			qc.andCondition(new QueryCondition(
					MedicalItemEntity.MATTERS_NEED_ATTENTION,
					QueryCondition.isNull, mattersNeedAttention_isNull));
		}
		if (mattersNeedAttention_isNotNull != null) {
			qc.andCondition(new QueryCondition(
					MedicalItemEntity.MATTERS_NEED_ATTENTION,
					QueryCondition.isNotNull, mattersNeedAttention_isNotNull));
		}
		if (mattersNeedAttention_in != null) {
			qc.andCondition(new QueryCondition(
					MedicalItemEntity.MATTERS_NEED_ATTENTION,
					QueryCondition.in, mattersNeedAttention_in));
		}
		return qc;
	}
