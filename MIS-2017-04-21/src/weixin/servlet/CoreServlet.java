			...
			if(MsgType.equals("event")){
				if(Event.equals("subscribe")){//第一次关注
					...
					if(subscribe != 0){
						...
						String reqmsg = "{\"action\":\"ADD_WECHAT_INFO_REQUEST\",\"content\":{\"openid\":\"" + openid + "\",\"nickname\":\"" + nickname + "\",";
						...
						if(result.equals("100")){//注册成功
							huifuString = XMLUtil.formatXmlAnswer(FromUserName,ToUserName,"感谢关注!【" + nickname + "】,您已成为我们的会员.您现在可以绑定需要关注的老人信息.");
				...
				if(Event.equals("VIEW")){//(已关注)正常进入微信公众号，用于拉取更新用户最新信息
					...
					if(subscribe != 0){
						...
						//根据openid获取用户id
						String reqmsgQuery = "{\"action\":\"QUERY_WECHAT_LIST_REQUEST\",\"page\":{\"pageno\":\"1\",\"pagesize\":\"1\"},\"content\":{\"openid\":\"" + openid + "\"}}";
						String queryId = SignUtil.doHttpPost(SignUtil.handlerurl + "/wechat.do?handlerWechat",reqmsgQuery);
						...
						if(resultQuery.equals("100")){
							...
							if (contentQuery != null && contentQuery.size() > 0) {
								JSONArray wechatList = contentQuery.getJSONArray("wechatList");
						...
						//根据微信用户表主键id更新基本信息
						if(!"0".equals(id)){
							String reqmsg = "{\"action\":\"ADD_WECHAT_INFO_REQUEST\",\"content\":{";
							reqmsg += "\"id\":" + id + ",";
				...
				if(Event.equals("unsubscribe")){//取消关注
					logger.debug(FromUserName +"   微信 取消关注");
				}
			}
		} catch (Exception e) {
